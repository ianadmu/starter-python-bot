# The persistance manager retrieves data from dropbox, and stores a copy in
# memory. The local copy can be interacted with, and updates will be sent to
# storage in regular intervals
# Note: do not interact with _data directly! it is a shared resource, and
# should be inrfaced with using get_data() and update_data()
...
class PersistanceManager(object):
    ...
    def _load_data(self):
        if (
            config_manager.config_loaded and
            config_manager.config["dropbox_access_token"] and
            self._data is None
        ):
            self.dropbox_client = dropbox.client.DropboxClient(
                config_manager.config["dropbox_access_token"]
            )
            self.data_lock.acquire()
            try:
                try:
                    f, metadata = self.dropbox_client.get_file_and_metadata(
                        self.file_name
                    )
                    self._data = f.read()
                except:
                    if self.seed_file:
                        ...
                        response = self.dropbox_client.put_file(
    ...
    def _run_backup(self):
        ...
        if self.dropbox_client and self.has_changed:
            ...
            stringFile = StringIO(
                unicode(
                    "".join(i for i in self._data if ord(i) < 128), "utf-8"
                )
            )
